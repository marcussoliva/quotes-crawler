...
class QuoteSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for quote in response.xpath('/html/body/div/div[2]/div[1]/div'):
            ...
            item_loader = ItemLoader(item=QuoteItem(), response=response)
            item_loader.add_value('text', quote.xpath('.//span[1]/text()').get())
            item_loader.add_value('author', quote.xpath('.//span[2]/small/text()').get())
